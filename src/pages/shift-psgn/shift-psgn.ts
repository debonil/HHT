...
getPassenger(coachValue){
  try{
    console.log("selected coach is :: "+JSON.stringify(coachValue.COACH_ID));
    var query={COACH_ID:coachValue.COACH_ID}
...
try{
    console.log("selected psgn is :: "+JSON.stringify(psgnValue));
...
getShiftType(shiftVal){
 console.log(JSON.stringify(shiftVal));
 var shiftType=JSON.stringify(shiftVal);
 if(shiftType.match("NORMAL SHIFT")){
   console.log("psgn to be shifted:: "+JSON.stringify(this.psgnValue));
