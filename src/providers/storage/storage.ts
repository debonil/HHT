...
export class StorageProvider {
      ...
      var collections = {
        ...
        passenger : {
          searchFields : {TRAIN_ID : 'integer', COACH_ID : 'string', BOARDING_PT : 'string', BERTH_INDEX : 'integer',
                          ...
                          CANCEL_PASS_FLAG : 'string', WAITLIST_NO : 'string'}
        }
      };
  ...
  replaceTrainAssignment(doc){
    return new Promise(resolve=>{
      let collectionName = 'trainAssignment';
      WL.JSONStore.get(collectionName).replace(doc).then((success)=>{
        resolve(true);
      },(failure)=>{
        resolve(false);
      });
  ...
  /*getWaitlistPassenger(collectionName){
    return new Promise(resolve=>{
      let options = { };
      WL.JSONStore.get(collectionName).find({BERTH_NO : -1}, {exact : true}).then((success)=>{
        var wlPsgn = [];
        success.forEach(element=>{
          wlPsgn.push(element);
        });
