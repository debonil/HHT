...
export class PsngDataServiceProvider {
 ...
 //   console.log("loadCoachwiseChartData CALLED by ==>" + caller);
  ...
  get syncStatus() {
    return new Promise(resolve => {
      this.storage.getAllDirtyDocuments(this.storage.collectionName.PASSENGER_TABLE).then(res => {
        if (res.length > 0) resolve("danger");
        this.storage.getAllDirtyDocuments(this.storage.collectionName.VACANT_BERTH_TABLE).then(res => {
          if (res.length > 0) resolve("danger");
        });
        this.storage.getAllDirtyDocuments(this.storage.collectionName.EFT_MASTER_TABLE).then(res => {
          if (res.length > 0) resolve("danger");
          resolve("light");
        });
      });
